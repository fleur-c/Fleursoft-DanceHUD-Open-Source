...
#define MessageStopAllDancing(releaseAllFlag)    llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlStop, (string) releaseAllFlag)
    // ReleaseAllFlag == TRUE - releases all dancers, FALSE - just stops and does NOT release dancers
#define xxxMessageSetAccurateTimer(flag)  llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlAccurateTimer, (string) flag)
#define MessageSyncAllDancers()    llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlSyncDancers, "")
#define MessageSendIM(msg)    llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlIM, msg)
#define MessageDCSetRecordingSequences(menuLinkid,flag)    llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlStartRecording, (string)menuLinkid + "|" + (string)flag)
#define MessageDCMissingAnimFlag(flag)    llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlMissingAnimWarning, (string) flag)
#define MessageDCDisableRepeatFlag(flag)    llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlDisableRepeat, (string) flag)
    // 0 = [repeat] works, 1 = [repeat] does NOT work
#define MessageDCSetDurationFlag(flag)    llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlSetDurationFlag, (string)flag)
    // flag = 0 for no default duration (this is the default for dance control), 1 = use default duration
#define MessageDCSetDurationValue(duration)    llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlSetDurationValue, (string)duration)
#define MessageDCStartNewDancer(dancerLinkId, avname, avkey, chatchannel, autoinvite)    llMessageLinked(LINK_THIS, Define_DANCECONTROLLINKID, (string)DanceControlLastDanceStartForDancer, llDumpList2String([dancerLinkId, avname, avkey, chatchannel, autoinvite], "|"))
